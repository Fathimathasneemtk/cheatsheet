...
* **Data Alignment**: AVX registers work most **efficiently when memory addresses are aligned to 32-byte boundaries**. Using `_mm256_load_ps` on unaligned memory will cause a segmentation fault (crash). Use `_mm256_loadu_ps` (the 'u' stands for unaligned) if you aren't sure, though it carries a minor performance penalty.
